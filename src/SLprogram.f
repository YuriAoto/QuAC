...
c Setup of number of grid points 
                  Mes = 500*(2**i) ! Mes must be an even integer
                  if(Mes.NE.2*(Mes/2)) then
                      write(*,*) ' No. mesh points Mes must be even !!!'
                      Mes = Mes + 1
                      endif  
                  Mes1 = Mes/2
                  gamma = DFLOAT(Mes)/DFLOAT(Mes1)
c======================================================================
                  IF(Imap.EQ.0) THEN 
c ... Integrate using one-parameter mapping function 
         CALL JLD(u_LJ_2n_n,Scale,C4,Rmin,Mes,Rbar,0.d0,Yl,SL_JLD)
         CALL JLD(u_LJ_2n_n,Scale,C4,Rmin,Mes1,Rbar,0.d0,Yl2,SL_JLD2)
                    ELSE
...
c     Set the mapping parameter beta
         ...
         CALL JLD(u_LJ_2n_n,Scale,C4,Rmin,Mes1,Rbar,beta,Yl2,SL_JLD2)
                    ENDIF
c======================================================================
c        Apply Richardson's extrapolation to zero step size  h->0 
c======================================================================
                  SL_err = (SL_JLD - SL_JLD2)/(gamma**4-1.d0)
                  SL_JLD_RE = SL_JLD + SL_err
c======================================================================         
c           Output of the resulting a_s-values 
c======================================================================
                  write(10,'(2I3,I8,3F18.12)') n,Imap,Mes,SL_JLD,
     1                                        SL_JLD_RE,SL_err
                  ENDDO
              ENDDO
          ENDDO
